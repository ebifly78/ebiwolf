...
def model_init(x, y, param, val, name):
    ...
    splitter = DefragModel.parseSLtrees(model)
    mdl = DefragModel(modeltype='classification', maxitr=100,
                      qitr=0, tol=1e-6, restart=20, verbose=0)
    mdl.fit(x, y, splitter, Kmax, fittype='FAB')
    joblib.dump(mdl, name + 'n.pkl')
...
limit_list = []
for i in range(20, 41):  # RFC de 42 kara yaru
            ...
            for param in param_list:
                for val in val_list:
                    ...
                    for d in range(1, days):
                        ...
                        out = open(
                            'csv/' + model_name + '_defrag_day{}.csv'.format(d), 'a')
